...
printf "Installing jq for mac to parse json output\nIf you have jq already installed then please comment out lint no: 4 and run the script again! \n"
brew install jq
printf "\n\n"
...
# Remove string quotes from token
tempToken=${token#'"'}
tempToken=${tempToken%'"'}
token=$tempToken
...
curl -H "$h1" -H "access-token: $token" "$url/api/banks?offset=1"
printf "\n\n"
...
curl -H "$h1" -H "access-token: $token" "$url/api/banksByName/ABHYUDAYA%20COOPERATIVE%20BANK%20LIMITED?limit=10"
printf "\n\n"
...
curl -H "$h1" -H "access-token: $token" "$url/api/banksByIfsc/ABHY0065001?limit=10"
printf "\n\n"
...
curl -H "$h1" -H "access-token: $token" "$url/api/banksByNameAndCity/ABHYUDAYA%20COOPERATIVE%20BANK%20LIMITED/MUMBAI"
printf "\n\n"
